...
class Command(BaseCommand):
    ...
    def _create_cz_accidents_poi(self, query):
        ...
        marker, created = Marker.objects.get_or_create(
            ...
            layer=overlay, slug="accident", minzoom=14, maxzoom=25,
        )
        ...
        for a in CzechiaAccidents.objects.filter(
            ...
            desc = (
                f"identifikační číslo: {a.identifikacni_cislo if a.identifikacni_cislo else null}<br/>"
                f"dátum: {a.datum if a.datum else null}<br/>"
                f"den: {a.den if a.den else null}<br/>"
                f"čas: {a.cas if a.cas else null}<br/>"
                f"druh nehody: {a.druh if a.druh else null}<br/>"
                f"druh srážky jedoucích vozidel: {a.druh_srazky_jedoucich_vozidel if a.druh_srazky_jedoucich_vozidel else null}<br/>"
                ...
                f"alkohol u vinika nehody přitomen: {a.alkohol_u_vinika_nehody_pritomen if a.alkohol_u_vinika_nehody_pritomen else null}<br/>"
                f"příčiny nehody: {a.priciny if a.priciny else null}<br/>"
                f"druh povrchu vozovky: {a.druh_povrchu_vozovky if a.druh_povrchu_vozovky else null}<br/>"
                f"stav povrchu vozovky v době nehody: {a.stav_povrchu_vozovky_v_dobe_nehody if a.stav_povrchu_vozovky_v_dobe_nehody else null}<br/>"
                f"stav komunikace: {a.stav_komunikace if a.stav_komunikace else null}<br/>"
                f"viditelnost: {a.viditelnost if a.viditelnost else null}<br/>"
                f"dělení komunikace: {a.deleni_komunikace if a.deleni_komunikace else null}<br/>"
                f"řízení provozu v době nehody: {a.rizeni_provozu_v_dobe_nehody if a.rizeni_provozu_v_dobe_nehody else null}<br/>"
                f"místní úprava přednosti v jizdě: {a.mistni_uprava_prednosti_v_jizde if a.mistni_uprava_prednosti_v_jizde else null}<br/>"
                f"specificka místa a objekty v místě nehody: {a.specificka_mista_a_objekty_v_miste_nehody if a.specificka_mista_a_objekty_v_miste_nehody else null}<br/>"
                f"směrové poměry: {a.smerove_pomery if a.smerove_pomery else null}<br/>"
                f"počet zúčastněných vozidel: {a.pocet_zucastnenych_vozidel if a.pocet_zucastnenych_vozidel else null}<br/>"
                f"místo dopravní nehody: {a.misto_dopravni_nehody if a.misto_dopravni_nehody else null}<br/>"
                f"druh pozemní komunikace: {a.druh_pozemni_komunikace if a.druh_pozemni_komunikace else null}<br/>"
                f"druh krizující komunikace: {a.druh_krizujici_komunikace if a.druh_krizujici_komunikace else null}<br/>"
                f"kategorie chodce: {a.kategorie_chodce if a.kategorie_chodce else null}<br/>"
                f"chování chodce: {a.chovani_chodce if a.chovani_chodce else null}<br/>"
                f"situace v místu nehody: {a.situace_v_miste_nehody if a.situace_v_miste_nehody else null}<br/>"
                f"situovani: {a.situovani if a.situovani else null}<br/>"
                f"druh_vozidla: {a.druh_vozidla if a.druh_vozidla else null}<br/>"
                f"charakteristika vozidla: {a.charakteristika_vozidla if a.charakteristika_vozidla else null}<br/>"
                f"smyk: {a.smyk if a.smyk else null}<br/>"
                f"vůz po nehodě: {a.vozidlo_po_nehode if a.vozidlo_po_nehode else null}<br/>"
                f"směr jízdy nebo postavení vozidla: {a.smer_jizdy_nebo_postaveni_vozidla if a.smer_jizdy_nebo_postaveni_vozidla else null}<br/>"
                f"stav řidice: {a.stav_ridice if a.stav_ridice else null}<br/>"
                f"vnější ovlivnění řidiče: {a.vnejsi_ovlivneni_ridice if a.vnejsi_ovlivneni_ridice else null}<br/>"
            )
